...
export default function SteamInsights({ name }) {
    ...
    if (!data) {
        return <div className="mt-6 h-[88px] rounded-xl bg-[#171a22] animate-pulse" aria-label={t('hub.steam.loading')} />;
    }
    if (!data.found) return null;

    const price = data.isFree ? t('hub.steam.freeToPlay') : data.price?.final;

    return (
        <div className="mt-6 rounded-xl border border-white/[0.06] bg-[#0e1016] overflow-hidden">
            {art?.found && art.hero && (
                <div className="relative">
                    <HubImage src={art.hero} alt={t('hub.steam.artwork', { name: data.name })} width={1280} className="w-full aspect-[96/31]" />
            ...
            <div className="p-4">
                <div className="flex items-center justify-between gap-3 mb-3">
                    <p className="gh-eyebrow">{t('hub.steam.liveOnSteam')}</p>
                    <a href={data.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-medium text-[#c4b5fd] hover:text-white">
                        {t('hub.steam.storePage')} <BsBoxArrowUpRight className="w-2.5 h-2.5" />
                    </a>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    <div>
                        <p className="text-xs text-[#6b7080]">{t('hub.steam.price')}</p>
                    ...
                    <div>
                        <p className="text-xs text-[#6b7080]">{t('hub.steam.playingNow')}</p>
                        <p className="text-sm font-semibold text-white mt-0.5 inline-flex items-center gap-1.5">
                            <BsPeopleFill className="text-[10px] text-emerald-400" /> {formatCount(data.players, locale)}
                        </p>
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                        <p className="text-xs text-[#6b7080]">{t('hub.steam.reviews')}</p>
                ...
                {data.news?.[0] && (
                    <a href={data.news[0].url} target="_blank" rel="noopener noreferrer" className="mt-3 pt-3 border-t border-white/[0.06] flex items-center justify-between gap-3 text-sm text-[#a1a6b3] hover:text-white">
                        <span className="truncate">{t('hub.steam.news', { title: data.news[0].title })}</span>
                        <span className="text-xs text-[#6b7080] shrink-0">{new Date(data.news[0].date).toLocaleDateString(locale)}</span>
                    </a>
                )}
            </div>
        </div>

...
export function UniverseCard({ universe, className = '' }) {
    return (
        <Link to={`/hub/${universe.id}`} className={`game-card group block ${className}`}>
            ...
            <p className="px-2.5 sm:px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs text-[#6b7080] truncate">Data: {universe.source}</p>
        </Link>
    );
}
...
export default function UniverseStrip() {
    ...
    if (!data) return <section className="mb-12"><SectionLoader title="Game universes" height="h-[180px]" /></section>;

    return (
        <section className="w-full mb-12">
            <SectionHeader
                title="Game universes"
                subtitle="Agents, champions, cards, shops and rotations straight from each game's API"
                action={<Link to="/hub" className="gh-btn gh-btn-secondary !h-9">View all</Link>}
            />

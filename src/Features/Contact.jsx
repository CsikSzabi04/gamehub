...
const channels = [
    { href: MAIL_URL, label: "Email", labelKey: "contact.email", detailKey: "contact.channels.email", icon: SiGmail },
    { href: "https://github.com/CsikSzabi04", label: "GitHub", detailKey: "contact.channels.github", icon: FaGithub },
    { href: "https://www.linkedin.com/in/szabolcs-cs%C3%ADk-a4b767315/", label: "LinkedIn", detailKey: "contact.channels.linkedin", icon: FaLinkedin },
    { href: "https://www.instagram.com/cs_szabj04/", label: "Instagram", detailKey: "contact.channels.instagram", icon: FaSquareInstagram },
];
    ...
    return (
        <>
            ...
            <main className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-16 min-h-[60vh]">
                <p className="gh-eyebrow mb-3">{t('contact.eyebrow')}</p>
                <h1 className="text-3xl sm:text-4xl font-extrabold text-white">{t('contact.title')}</h1>
                <p className="mt-3 text-[#a1a6b3] max-w-xl">
                    {t('contact.intro')}
                </p>

                <ul className="mt-8 sm:mt-10 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {channels.map(({ href, label, labelKey, detailKey, icon: Icon }) => (
                        <li key={label}>
                            <a
                                href={href}
                                target="_blank"
                            ...
                            >
                                ...
                                <span className="min-w-0 flex-1">
                                    <span className="block text-sm font-semibold text-white">{labelKey ? t(labelKey) : label}</span>
                                    <span className="block text-sm text-[#8a8f9c] truncate">{t(detailKey)}</span>
                                </span>

...
export default function ShowCards({ selectedGame, closeModal, modalVisible }) {
  ...
  async function delFav(id){
    ...
    const fav = { name: selectedGame.name, userId: user.uid };
    //console.log(id)
    const resp = await fetch("https://gamehub-backend-zekj.onrender.com/delfav/"+id)
  }
  ...
  return (
    <div className="modal show fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50" id="game-modal">
      <div className="modal-content rounded-lg sm:max-w-lg mx-4 sm:mx-0 p-6 sm:p-8 max-h-[90%]">
        ...
        {fav == true ?(
           <div><span className="add-button 0 text-white px-4 py-2 rounded-md cursor-pointer" onClick={addFav}>Add to Fav</span><br /></div>
        ):<div><span className="del-button 0 text-white px-4 py-2 rounded-md cursor-pointer" onClick={()=>delFav(selectedGame.id)}>Delete from Fav</span><br /></div>}

...
export default function Header({ searchTrue, setGames, setSearchTrue, games }) {
    ...
    async function getStores() {
      try {
        const resp = await fetch(
          "https://gamehub-backend-zekj.onrender.com/stores"
        );
  ...
  useEffect(() => {
    async function getFavorites() {
      try {
        const resp = await fetch(
          `https://gamehub-backend-zekj.onrender.com/getFav?userId=${user?.uid}`
        );
        const data = await resp.json();
        setFavorites(data);
      } catch (error) {
    ...
    <>
      <header className=" bg-gray-800 text-white p-4 md:p-8">
        <div className="header-container mx-auto px-6 md:px-12 w-full flex flex-col md:flex-row justify-between items-center">
          <div className="logo-container mb-4 md:mb-0 text-center md:text-left mr-5">
            <Link to="/">
              <div className="inline-flex gap-2">
                ...
                <h1 className="text-2xl md:text-3xl font-bold text-sky-500 cursor-pointer">
                  Data
                </h1>
                <h1 className="text-2xl md:text-3xl font-bold cursor-pointer">
                  Hub
          ...
          />
          <button
            className="menu-button text-white text-2xl ml-5 mr-5"
          ...
          <nav
            className={`nav-links w-full xl:flex xl:items-center xl:space-x-4 mt-4 xl:mt-0 ${
              menuOpen ? "block" : "hidden"
            }`}
          >
            <div className="flex flex-col xl:flex-row xl:space-x-4">
              <button
                className="nav-button text-white px-4 py-2 rounded-lg w-full xl:w-auto"
                onClick={() => setStoreVisible(true)}
              >
                Stores
              </button>
              <a href="#news">
                <button className="nav-button text-white px-4 py-2 rounded-lg w-full xl:w-auto">
                  News
                </button>
              </a>
              <Link to="/discover">
          ...
          <div className="mt-4 xl:mt-0 flex xl:float-right">
            ...
            {user ? (
              <Link to="/profile">
                <button className=" text-white px-4 py-2 rounded-lg w-full xl:w-auto text-lime-600 text-4xl">
                  <CgProfile />
                </button>
              </Link>
            ) : (
              <Link to="/login">
                <button className="nav-button flex text-white px-4 py-2 rounded-lg w-full xl:w-auto">
                  Login <CiLogin className="ml-1 mt-1" />
                </button>
              </Link>
            )}
          </div>
      ...
      {modalStoreVisible && (
        <div className="modal show fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50">
          <div className="modal-content text-white p-6 rounded-lg">
            <span
              className="close-button text-2xl absolute top-2 right-2 cursor-pointer"
        ...
        <div className="modal show fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50">
          <div className="modal-content text-white p-6 rounded-lg">
            ...
            {favorites.length === 0 ? (
              ...
              <div className="store overflow-y-auto max-h-96">
                {favorites.map((fav, i) => (
                  <div key={i} className="fav-card bg-gray-800  rounded-md ">
                    <p className="storename text-white p-2 flex items-center ">
                      {fav.name}{" "}
                      <TiDelete
...
/* fav-card bg-gray-800 p-4 rounded-md mb-4 */

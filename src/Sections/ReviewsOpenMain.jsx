                                    ...
                                    {game.reviews.length > 0 && (
                                        <div className="bg-gray-700 p-2 sm:p-3 rounded">
                                            <div className="flex items-center mb-1">
                                                <span className="text-xs sm:text-sm font-medium">{game.reviews[0].email}</span>
                                                <div className="flex ml-2">
                                                    {[...Array(5)].map((_, i) => ( <span key={i} className={`text-xs ${i < game.reviews[0].rating ? 'text-yellow-400' : 'text-gray-400'}`} >   ★ </span>))}
                                                </div>
                                            </div>
                                            <p className="text-xs sm:text-sm text-gray-300 line-clamp-2">{game.reviews[0].review}</p>
                                        </div>
                                    )}
                                </div>
                            </div>

...
export default function Notfound() {
    ...
    function openStoreUrl(name){
       ...
       let url="";
        ...
        if (name.toLowerCase() == 'steam') {
            ...
            url = `${store.storeName.toLowerCase().replace(/\s+/g, '')}.com`;
        }
    ...
    return (
        <div>
            ...
            { modalStoreVisible && (
                <div className="modal show" id="game-modal">
                <div className="modal-content" >
                    <span className="close-button" onClick={closeStore}>&times;</span>
                    <div className='store' >
                    {store.map((x,i)=> <div key={i}> {/*<img className='store-pic' src={x.image.logo}/>*/} <p onClick={e=>openStoreUrl(x.storeName)} className='store-row' key={x.storeID}>{x.storeName} </p></div>)}
                    </div>
                </div>
            </div>
            )}

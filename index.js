...
const center = document.getElementById("center-main");
...
async function displayMainGames(filteredGames2 = allGames2){   
     
    const response2 = await fetch(`${API_URL}?key=${API_KEY}&page_size=20`);
    const data2 = await response2.json();
    allGames2 = data2.results2; 

    center.innerHTML = "";
    // Loop through each game and create a card for it.
    filteredGames2.forEach((game) => {
        const gameCard = document.createElement("div");
        gameCard.className =
            "main-card hover:shadow-xl hover:scale-105 transition transform cursor-pointer";
        gameCard.onclick = () => showGameDetails(game); // Show details on click
 
         // Set the game card's HTML.
        gameCard.innerHTML = ` 
            <img src="${game.background_image}" alt="${game.name}" class="main-game-image">
        ...
        `;
        center.appendChild(gameCard); //Ads gamecard 
    });
}
